...
describe('Matrix', () => {
  test('extract _rows from one number matrix', () => {
    expect(new Matrix('1').rows[0]).toEqual([1]);
  });

  test('can extract _rows', () => {
    expect(new Matrix('1 2\n3 4').rows[1]).toEqual([3, 4]);
  });

  test('extract _rows where numbers have different widths', () => {
    expect(new Matrix('1 2\n10 20').rows[1]).toEqual([10, 20]);
  });

  test('can extract _rows from non-square matrix with no corresponding column', () => {
  ...
  test('can extract column from non-square matrix with no corresponding _rows', () => {
